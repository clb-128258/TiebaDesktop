...
class Follow:
    ...
    Attributes:
        ...
        user_name (str): 用户名
        nick_name_new (str): 旧版昵称
        desp (str): 个性签名
        forum_admin_info (str): 用户担任的吧主信息 如果用户没有担任吧主则为空
    ...
    def from_tbdata(data_map: Mapping) -> Follow:
        ...
        user_name = data_map.get('name')
        nick_name_new = data_map["name_show"]
        desp = data_map['intro']
        if not data_map['bazhu_grade']:
            forum_admin_info = ''
        else:
            forum_admin_info = data_map['bazhu_grade']['desc']
        return Follow(user_id, portrait, user_name, nick_name_new, desp, forum_admin_info)
...
class Follows(TbErrorExt, Containers[Follow]):
    ...
    def from_tbdata(data_map: Mapping) -> Follows:
        objs = [Follow.from_tbdata(m) for m in data_map.get("follow_list", [])]
