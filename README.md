...
需要提醒的是，由于 Github Actions 分配虚拟机资源存在延迟，实际上构建流程会延迟十几分钟才执行。
### 如何获取
...
> [!note]
>
> 1) 目前 CI 流程**只支持 64 位 Windows 系统**，其它架构与系统敬请期待；
> 2) CI 构建产物的**留存有效期为 2 天**，超过 2 天后会被 Github 自动删除。本仓库也会定期自动清理旧的 Workflows。
