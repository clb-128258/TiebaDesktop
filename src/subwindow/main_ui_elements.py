"""
主页面核心 UI 的承载库（包括主窗口，托盘图标，设置窗口等）
"""
import gc
import os
import platform
...
class TrayIcon(QSystemTrayIcon):
    ...
    def show_balloon_message(self, title, text, icon, timeout):
    ...
    def update_tooltip(self):
        ...
        account = self.main_window.account_manager.current_account
        if account:
            base_text += f'\n当前账号: {account.nickname}'

        interact_syncer = self.main_window.notice_syncer
    ...
    def handle_click(self, tpe):
        if tpe != QSystemTrayIcon.ActivationReason.Context:
            self.main_window.show_active_window()

    def init_menu(self):
        self.menu = base_ui.BaseQMenu()

        open_main_window = QAction('打开主窗口', self)
        open_main_window.triggered.connect(self.main_window.show_active_window)
        ...
        open_settings_window.triggered.connect(self.main_window.open_settings_window)
        self.menu.addAction(open_settings_window)

        exit_app = QAction('退出软件', self)
        exit_app.triggered.connect(self.main_window.exit_app)
    ...
    def show_into_forum_page(self):
        self.main_window.show_active_window()
        self.main_window.switch_follow_forum_page()

    def show_interact_page(self):
        self.main_window.show_active_window()
        self.main_window.switch_interact_page()

    @classmethod
    def create_instance(cls):
        global tray_icon_instance
        if not tray_icon_instance:
            tray_icon_instance = TrayIcon()
